...
没有"源码/预览"切换：标题、引用、列表、表格、任务清单等块级结构始终以排版后
形态呈现与编辑——标题保持层级字号（级别用 Ctrl+0-6 调整）、列表显示项目符号、
表格点开单元格直接改内容；粗体、斜体、行内代码、链接等行内标记在光标进入时
显示原文，随写随改。数学公式（KaTeX）、删除线等 GFM 扩展同样支持，导出 HTML
时样式与图片全部内联。
