...
(function () {
  ...
  function fileRow(entry) {
    ...
    const classes = ["file-row", "draggable-file", open ? "is-open" : "", missing ? "is-missing" : ""]
      .filter(Boolean).join(" ");
    const end = missing
      ? '<span class="row-end"><button class="row-action" title="重新定位">' + ICON.warn.replace('class="warn"', "") + "</button></span>"
      : '<span class="row-end"><span class="row-time">' + D.relativeTime(entry.lastOpenedMs) + "</span>" +
        '<button class="row-action row-action-remove" title="移除">' + ICON.x + "</button></span>";
    ...
    return (
      ...
      '<span class="row-name">' + name + "</span>" + end + "</div>"
    );
  }
  ...
  function render() {
    ...
    for (const entry of d.recent) {
      const bucket = D.timeBucket(entry.lastOpenedMs);
      if (!buckets.has(bucket.key)) {
        buckets.set(bucket.key, { label: bucket.label, items: [] });
      }
      buckets.get(bucket.key).items.push(entry);
    }
    for (const [, bucket] of buckets) {
      html += '<div class="bucket-title">' + bucket.label + "</div>";
      html += bucket.items.map(fileRow).join("");
    }
    ...
    for (const group of d.groups) {
      ...
      if (group.expanded) {
        html += '<div class="group-children">' +
          group.paths.map((p) => fileRow({ path: p })).join("") +
          "</div>";
      }
    }
