...
## 10. 实施记录（阶段 0–5，全部完成，2026-09-11）
...
改动文件：`preview/markdown.ts`（正文 16px/1.75、标题梯度、代码块与行内代码去边框、引用中性左条、表头、预览底色）、`editor/extensions.ts`（行高基准、内容内边距、行号宽度、markdown 源码列宽）、`state/preferences.ts`（默认字号）、`styles/tokens.css`（`--fs-editor`、当前行与选区）、`styles/app.css`（浮动切换按钮），测试与快照同步更新。

| 指标 | 实施前 | 实施后 |
|---|---|---|
| 渲染正文字号 / 行高 | 15px / 27px | **16px / 28px** |
| 源码正文字号 / 行高 | 18px / 26.9px | **16px / 28.05px** |
| 渲染正文列宽 | 582px（72ch） | 638px（74ch） |
| 源码正文列宽 | 全宽（约 730px） | 676px（74ch，行号槽随内容居中） |
| 代码文件 | 全宽 | 全宽（实测 722 / 796px，无列宽限制） |
| 预览 H1 / H2 | 2em(30px) / 1.5em，字重 700 | 1.75em(28px) / 1.35em，字重 650 |
| 行内代码 / 代码块 | 0.875em + 1px 边框 | 0.84em(13.4px)，纯底色无边框 |
| 引用块左条 | 2.5px 混入 accent | 3px 中性 border-strong |
| 预览页底色 | 侧栏色 `#f9f9fb` | 内容面 `#ffffff`（与源码模式一致，切换不闪） |
| 浮动切换按钮 | 药丸：边框 + 阴影 + 半透明底 | 浮标：无边框无阴影、tertiary 文字，悬停才浮底 |
| 当前行 / 选区 | 4% / 22% | 3.5% / 18%（深色同步调轻） |

**顺带修复（重要）**：**切标签后旧编辑器仍然可见**。M6 引入的 class 隐藏方案与 CodeMirror 冲突——`EditorView.updateAttrs()` 在 focus/blur 时**整体重写 `.cm-editor` 的 class**（`cm-focused` + 主题类），把 `cm-doc-hidden` 一并冲掉；切标签恰好触发焦点转移，于是旧编辑器保持可见并与当前编辑器纵向堆叠（当前文档被顶出可视区）。修复方式：隐藏标记改用 CM 不管理的 `data-doc-hidden` 属性（`app.css` 属性选择器 + `EditorPane.syncVisibility()` 按 DOM 同步 + 下一帧兜底重同步 + `registry.replaceContent` 保留该属性），`tests/editorVisibility.test.ts` 更新为守护这条不变量。实测 7 步切换序列全部满足「可见编辑器 = 活动标签」。

**验收**：源码与渲染字号行高一致（16px / 28px），列宽差 38px（6%，源于两种字体 ch 宽度不同）；代码文件全宽；`npm test` 123 全绿、`tsc` 通过；截图 `docs/ui-redesign/after-p3/`（浅色 12 张 + 深色 6 张），度量见 `docs/ui-redesign/type-after-p3.json`。

### 阶段 4 · 独立窗口与浮层（已完成）

改动文件：`styles/primitives.css`（浮层去实色描边、对话框按钮进 32px 档）、`styles/app.css`（对话框圆角/内边距/字阶、设置标题与行高、开关尺寸、查找内边距与选项芯片、编解码文本域与芯片、垃圾桶与"恢复默认"归到 24px 档）、`styles/tokens.css`（`--fs-settings-title`）、`components/SettingsWindow.tsx`（字号"恢复默认"对齐新默认值 12）。

| 指标 | 实施前 | 实施后 |
|---|---|---|
| 对话框面板 | 圆角 8px / 内边距 20px | 圆角 12px / 内边距 24px |
| 对话框按钮 | 30px | **32px**（`--ctl-lg`） |
| 对话框标题 / 消息 | 13px / 12px、行高 1.5 | 14px（`--fs-title`）/ 13px、行高 1.65 |
| 浮层描边 | 1px 实色 border + 阴影 | **仅阴影**（首层自带 1px 发丝线，去掉"到处是框"） |
| 设置页标题 | 20px | 16px |
| 设置行高 | 65px | 57px |
| 开关 | 40×22（滑块 16px） | 32×18（滑块 14px） |
| "恢复默认" / 侧栏垃圾桶 | 23px / 20px | 24px（`--ctl-sm`） |
| 查找窗口内边距 | 24px | 16px |
| 查找选项 | 18px 行高的裸 checkbox | 24px 芯片（悬停底色，保留中文标签） |
| 查找结果计数 | 普通数字、靠左 | 靠右（`margin-left:auto`）+ `tabular-nums` |
| 编解码文本域 | 12px / 行高 1.5 | 13.5px（`--fs-mono`）/ 行高 1.6 |
| 编解码操作芯片 | 内边距撑出（不统一） | 24px（`--ctl-sm`） |

**实测**（`tools/tiers.mjs`，逐界面枚举控件高度）：对话框按钮 32 / 主窗口工具栏 28 / 标签"+"与"清除最近文件" 24；查找窗口输入与按钮 28、选项芯片 24；设置页下拉 28、"恢复默认" 24、开关轨道 18（设计值）、行高 57px；编解码操作芯片 24、动作按钮 28。**结论：独立按钮/输入/下拉/文本域只剩 24 / 28 / 32 三档**（分段控件内按钮 22px、步进器内按钮 26px 属复合控件内部尺寸，按设计保留）。

**偏差说明**：查找窗口的复选项保留中文标签做成 24px 芯片，未按方案原文改成 "Aa / .*" 图标按钮——`.*` 在编辑器语境通常表示正则，而本项语义是"解释转义字符"，用图标会误导。

### 阶段 5 · 走查与基线（已完成）

- **三项测试**：`npm test` 123 全绿、`npm run check`（tsc）通过、`cargo test` **48 全绿**（`perf_baseline` 按设计 `#[ignore]`）。
- **键盘走查**（`tools/focus.mjs`）：从侧栏开关 → 标签"+" → 撤销/重做 → 语言 → 视图模式 → 窗口控制 → 侧栏操作 → 设置 → 预览工具条，共 14 个可聚焦元素，Tab 顺序与视觉顺序一致；每个命中 `:focus-visible`，焦点环为 1.83px 强调色（60% 透明）内描边、offset -1.83px。
- **最终对比度复测**（`tools/contrast.mjs`，含半透明背景合成）：浅色 4.64–14.58、深色 4.72–15.30，**全部 ≥ 4.5:1**。
- **基线重拍**：`docs/ui-redesign/final/`（18 张：浅色主窗 11 + 欢迎页 1 + 深色 6）并附 `contrast-light.json` / `contrast-dark.json`。
